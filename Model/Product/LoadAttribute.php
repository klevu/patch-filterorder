 ...
 */
namespace Klevu\Filterorder\Model\Product;
...
use \Magento\Eav\Model\ResourceModel\Entity\Attribute\Option\Collection as Klevu_Product_Attribute_Value_Collection;

class LoadAttribute extends  AbstractModel implements \Klevu\Search\Model\Product\LoadAttributeInterface
{
	...
	protected $_productAttributeValueCollection;
    ...
    public function __construct(
		...
		Klevu_Product_Attribute_Value_Collection $productAttributeValueCollection
		...
		$this->_productAttributeValueCollection = $productAttributeValueCollection;
    }
                        ...
                        foreach ($attr->setStoreId($this->_storeModelStoreManagerInterface->getStore()->getId())->getSource()->getAllOptions(false) as $option) {
                            if (is_array($option['value'])) {
                                foreach ($option['value'] as $sub_option) {
                                    if (!empty($sub_option)) {
										if($attr->getAttributeCode() != "visibility")
										{
											$suboptionSortOrder = $this->_productAttributeValueCollection
											->setIdFilter($sub_option['value'])
											->getFirstItem()
											->getSortOrder();
											if(empty($suboptionSortOrder)){
												$suboptionSortOrder = 0;
											}
											$suboptionSortOrder = sprintf("%'.05d\n", $suboptionSortOrder);
											$attribute_data[$attr->getAttributeCode()]['values'][$sub_option['value']] = $suboptionSortOrder."__".$sub_option['label'];
										}else{
								...
								if($attr->getAttributeCode() != "visibility")
								{
									$optionSortOrder = $this->_productAttributeValueCollection
										->setIdFilter($option['value'])
										->getFirstItem()
										->getSortOrder();
									if(empty($optionSortOrder)){
										$optionSortOrder = 0;
									}
									$optionSortOrder = sprintf("%'.05d\n", $optionSortOrder);
									$attribute_data[$attr->getAttributeCode()]['values'][$option['value']] = $optionSortOrder."__".$option['label'];
								}else{
									$attribute_data[$attr->getAttributeCode()]['values'][$option['value']] = $option['label'];
								}	
                            }
							$this->_productAttributeValueCollection->clear()->getSelect()->reset(\Zend_Db_Select::WHERE);
                        }
                    }
                }
